...
def open_url(host: str, port: int):
    flag = True
    while flag:
        logger.info('start Perf Monitor X server ...')
        f = Figlet(font="slant", width=300)
        print(f.renderText("Perf Monitor X {}".format(__version__)))
        print(f.renderText("Taby" ))
...
def main(host=ip(), port=50003):
    try:
        ...
        logger.info('stop Perf Monitor X success')
        sys.exit()
